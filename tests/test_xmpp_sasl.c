...
static int setup_test_db(const char **db_path_out) {
    char path[512];
    snprintf(path, sizeof(path), "/tmp/test_xmpp_%d_%d.db",
    ...
    const char *insert_sql =
        "INSERT INTO users (jid, password_plain, created_at, disabled) "
        "VALUES (?, ?, ?, ?)";
    sqlite3_stmt *stmt;
...
static int feed_sasl_plain(xmpp_session_t *ctx,
...
static void test_xmpp_stream_negotiation(void **state) {
    ...
    int rc = xmpp_feed(&ctx, client_hello, strlen(client_hello),
                       mock_write, NULL);
...
static void test_xmpp_full_connection_flow(void **state) {
    ...
    /* Step 3: Stream restart — RFC 6120 §4.3.3: no <?xml?> declaration on restart. */
    const char *client_restart =
        "<stream:stream xmlns:stream='http://etherx.jabber.org/streams' "
        "xmlns='jabber:client' to='localhost' version='1.0' "
        "xml:lang='en'>";
    rc = xmpp_feed(&ctx, client_restart, strlen(client_restart),
                   mock_write, NULL);
    assert_int_equal(rc, 0);
...
static void test_xmpp_disabled_account(void **state) {
    ...
    snprintf(sql, sizeof(sql),
             "UPDATE users SET disabled = 1 WHERE jid = 'testuser@localhost'");
    sqlite3 *db;
    ...
    const char *client_hello =
        ...
        "xmlns='jabber:client' to='localhost' version='1.0' "
        "xml:lang='en'>";
    rc = xmpp_feed(&ctx, client_hello, strlen(client_hello),
                   mock_write, NULL);
    assert_int_equal(rc, 0);
...
static void test_xmpp_incomplete_elements(void **state) {
    ...
    const char *chunk1 = "<?xml version='1.0'?><stream:stream xmlns:stream='http://etherx.jabber.org/streams' xmlns='jabber:client' to='localhost' version='1.0' xml:lang='en'>";
