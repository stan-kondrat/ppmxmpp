...
static int setup_test_config(const char *host, int port) {
    char conf_path[512];
    char db_path[512];
    snprintf(conf_path, sizeof(conf_path), "/tmp/test_server_conf_%d_%d_%d.conf",
             getpid(), (int)time(NULL), rand());
    snprintf(db_path, sizeof(db_path), "/tmp/test_server_%d_%d.db",
...
/* server_start() with a non-IP host string should return -1 (uv_ip4_addr fails). */
