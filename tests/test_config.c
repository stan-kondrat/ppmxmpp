 ...
 * ------------------------------------------------------------------------ */
static void remove_conf_file(const char* path) { unlink(path); }

/* ---------------------------------------------------------------------------
 * Helper: remove a directory if it exists (silent on error).
 * ------------------------------------------------------------------------ */
static void remove_dir(const char* dir) { rmdir(dir); }
...
static void test_config_created_with_nested_dir(void** state) {
  ...
  char path[512];
  char dir[512];
  snprintf(path, sizeof(path), "/tmp/test_xmpp_%d_%d/subdir/config.conf", getpid(),
           (int)time(NULL));
