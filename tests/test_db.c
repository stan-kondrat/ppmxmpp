...
static void gen_db_path(char *buf, size_t len) {
    snprintf(buf, len, "/tmp/test_xmpp_db_%d_%d_%d.db",
             getpid(), (int)time(NULL), rand());
}
...
static int setup_test_config(const char *db_path) {
    char conf_path[512];
    snprintf(conf_path, sizeof(conf_path), "/tmp/test_xmpp_conf_%d_%d.conf",
...
static void test_db_prepare_select(void **state) {
    ...
    assert_int_equal(storage_db_prepare(db,
        "SELECT jid, password_plain FROM users WHERE jid = ?", &stmt), 0);
...
static void test_db_prepare_update(void **state) {
    ...
    storage_stmt_t *stmt = NULL;
    assert_int_equal(storage_db_prepare(db,
        "UPDATE users SET disabled = 1 WHERE jid = ?", &stmt), 0);
    assert_non_null(stmt);
...
static void test_db_prepare_cached(void **state) {
    ...
    assert_int_equal(storage_db_prepare(db,
        "SELECT jid FROM users WHERE jid = ?", &stmt1), 0);
    assert_int_equal(storage_db_prepare(db,
        "SELECT jid FROM users WHERE jid = ?", &stmt2), 0);
...
static void test_db_bind_text_select(void **state) {
    ...
    assert_int_equal(storage_db_prepare(db,
        "SELECT jid FROM users WHERE jid = ?", &select_stmt), 0);
...
static void test_db_column_text_copy(void **state) {
    ...
    storage_stmt_t *select_stmt = NULL;
    assert_int_equal(storage_db_prepare(db,
        "SELECT password_plain FROM users WHERE jid = ?", &select_stmt), 0);
...
static void test_db_column_text_null(void **state) {
    ...
    storage_stmt_t *stmt = NULL;
    assert_int_equal(storage_db_prepare(db,
        "SELECT jid FROM users WHERE jid = ?", &stmt), 0);
...
static void test_db_changes_update(void **state) {
    ...
    assert_int_equal(storage_db_prepare(db,
        "UPDATE users SET disabled = 1 WHERE jid = ?", &update_stmt), 0);
...
int main(void) {
    const struct CMUnitTest tests[] = {
        /* Open / close tests. */
        cmocka_unit_test_setup_teardown(test_db_open_creates_file,
                                        db_test_setup, db_test_teardown),
        cmocka_unit_test_setup_teardown(test_db_open_singleton,
                                        db_test_setup, db_test_teardown),
        cmocka_unit_test_setup_teardown(test_db_open_invalid_path,
                                        db_test_setup, db_test_teardown),
        cmocka_unit_test_setup_teardown(test_db_reopen_after_close,
                                        db_test_setup, db_test_teardown),

        /* Prepare tests. */
        cmocka_unit_test_setup_teardown(test_db_prepare_select,
                                        db_test_setup, db_test_teardown),
        cmocka_unit_test_setup_teardown(test_db_prepare_insert,
                                        db_test_setup, db_test_teardown),
        cmocka_unit_test_setup_teardown(test_db_prepare_update,
                                        db_test_setup, db_test_teardown),
        cmocka_unit_test_setup_teardown(test_db_prepare_cached,
                                        db_test_setup, db_test_teardown),
        cmocka_unit_test_setup_teardown(test_db_prepare_invalid_sql,
                                        db_test_setup, db_test_teardown),

        /* Bind / step / column tests. */
        cmocka_unit_test_setup_teardown(test_db_bind_text_insert,
                                        db_test_setup, db_test_teardown),
        cmocka_unit_test_setup_teardown(test_db_bind_text_select,
                                        db_test_setup, db_test_teardown),
        cmocka_unit_test_setup_teardown(test_db_bind_int64,
                                        db_test_setup, db_test_teardown),
        cmocka_unit_test_setup_teardown(test_db_column_text_copy,
                                        db_test_setup, db_test_teardown),
        cmocka_unit_test_setup_teardown(test_db_column_text_null,
                                        db_test_setup, db_test_teardown),

        /* Reset tests. */
        cmocka_unit_test_setup_teardown(test_db_reset_clears_bindings,
                                        db_test_setup, db_test_teardown),
        cmocka_unit_test(test_db_reset_null),

        /* Bind null tests. */
        cmocka_unit_test_setup_teardown(test_db_bind_null,
                                        db_test_setup, db_test_teardown),
        cmocka_unit_test(test_db_bind_null_stmt),

        /* Column int64 tests. */
        cmocka_unit_test(test_db_column_int64_null),
        cmocka_unit_test_setup_teardown(test_db_column_int64_valid,
                                        db_test_setup, db_test_teardown),

        /* Changes tests. */
        cmocka_unit_test_setup_teardown(test_db_changes_insert,
                                        db_test_setup, db_test_teardown),
        cmocka_unit_test_setup_teardown(test_db_changes_update,
                                        db_test_setup, db_test_teardown),

        /* Migration tests. */
        cmocka_unit_test_setup_teardown(test_db_migration_creates_users_table,
                                        db_test_setup, db_test_teardown),
        cmocka_unit_test_setup_teardown(test_db_no_remigration,
                                        db_test_setup, db_test_teardown),
        ...
        cmocka_unit_test_setup_teardown(test_db_path_configured,
                                        db_test_setup, db_test_teardown),
    };
