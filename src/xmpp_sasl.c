...
static int jid_localpart_has_forbidden(const char *s) {
    ...
    for (; *s; s++) {
        if (strchr(forbidden, *s)) return 1;
    }
    return 0;
}
...
int handle_sasl_plain(xmpp_session_t *ctx, const char *b64_text,
    ...
    int ac_len = (int)(strlen(authcid) < sizeof(ctx->authcid) - 1 ? strlen(authcid) : sizeof(ctx->authcid) - 1);
    ...
    /* password is not NUL-terminated — it occupies the tail of the decoded buffer. */
    ...
    /* RFC 4616 §2: if authzid is non-empty it must equal the derived identity (bare_jid). */
    if (authzid_len > 0) {
