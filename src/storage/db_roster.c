...
int storage_roster_list(const char* owner_jid, storage_roster_item_cb cb, void* ud) {
  ...
  while ((rc = storage_db_step(stmt)) == SQLITE_ROW) {
    ...
    if (cjid) {
      strncpy(item.contact_jid, cjid, sizeof(item.contact_jid) - 1);
      free(cjid);
    }
    if (name) {
    ...
    if (sub) {
      strncpy(item.subscription, sub, sizeof(item.subscription) - 1);
      free(sub);
    }
    item.ask = (int)ask;
...
int storage_roster_get(const char* owner_jid, const char* contact_jid,
  ...
  if (rc == SQLITE_ROW) {
    ...
    if (cjid) {
      strncpy(item_out->contact_jid, cjid, sizeof(item_out->contact_jid) - 1);
      free(cjid);
    }
    if (name) {
      strncpy(item_out->name, name, sizeof(item_out->name) - 1);
      free(name);
    }
    if (sub) {
      strncpy(item_out->subscription, sub, sizeof(item_out->subscription) - 1);
      free(sub);
    }
